...
function fileDialogStart() {
	...
	container.innerHTML = '<span class="legend">{{ =_( "Photos Queue" )}}</span>';
}   
   
function fileQueued(file) {
	try {
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("{{ =_( 'Pending...' )}}");
	...
	try {
		if (numFilesSelected > 0) {
			...
			if(albumname)
			{
				...
				status.innerHTML = "{{ =_( 'Photos are uploading to album' )}} [" + albumname + "] 0/" + numFilesSelected;
				successcount = 0;
...
function uploadStart(file) {
	try {
		 ...
		 */
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("{{ =_( 'Uploading...' )}}");
		progress.toggleCancel(true, this);
	}
	catch (ex) {}
...
function uploadSuccess(file, serverData) {
	try {
		...
		if(result.result == 'ok')
		{
			...
			successcount++;
			var status = document.getElementById("divStatus");
			status.innerHTML = "{{ =_( 'Photos are uploading to album' )}} [" + albumname + "] " +
		...
		progress.setError();
		progress.toggleCancel(false);
		progress.setStatus("{{ =_( 'Upload Failed' )}}");
	}
}
...
function queueComplete(numFilesUploaded) {
	...
	var albumselect = document.getElementById("albumsSelect");
	albumname = albumselect.value;
	status.innerHTML = successcount + ' {{ =_( "Photos are uploaded to album" )}} <a target="_blank" href="/' + albumname + '">' + albumname + '</a>';
